# YOLACT RegNetX-800MF：Raspberry Pi 5 / CM5 + Hailo-8 实例分割

本模块运行 YOLACT-RegNetX-800MF 实例分割（COCO 80 类，UC Davis），后处理在
...
| 端点 | 方法 | 说明 |
|---|---|---|
| `/api/models/yolact_regnetx_800mf/predict` | POST | 检测结果 + 每目标 mask 面积（JSON） |
...
（模型 `yolact_regnetx_800mf`，源：[dbolya/yolact](https://github.com/dbolya/yolact)）。