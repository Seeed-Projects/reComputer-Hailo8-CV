...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (1920, 1024)  # (width, height) must match the .hef input shape
...
async def index():
    ...
    <html>
      ...
      <body>
        <div class="container">
          <h1>CM5 + Hailo-8 STDC1 Semantic Segmentation</h1>
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    if not _SEG_OUTPUT_LOGGED:
        ...
        print(
            f"[STDC1] raw output shape={raw_shape}, decoded mask shape={class_mask.shape}, "
...
def main():
    parser = argparse.ArgumentParser(description='STDC1 semantic segmentation on CM5 + Hailo-8 (Web Preview Mode)')
