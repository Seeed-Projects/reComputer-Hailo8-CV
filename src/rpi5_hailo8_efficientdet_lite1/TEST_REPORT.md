# Test Report — EfficientDet-Lite1 on CM5 + Hailo-8

## Model

| Field | Value |
|---|---|
| Model name | efficientdet_lite1 |
| Task | Object detection (COCO 80 thing classes) |
| Backbone | EfficientDet-Lite1 (BiFPN) |
| Parameters | 4.73M |
| Operations | 4G |
...
| Sibling | efficientdet_lite0 (same I/O contract, lighter backbone, 320x320 input) |

## HEF artifact

| Field | Value |
|---|---|
| File | model/efficientdet_lite1.hef |
| Size | ~16 MB (16,223,949 bytes) |
| SHA256 | ee2546d045303dc00d732e250069a1cb3d340eb897e1da43c538b75fbaf25f5d |
| Source | https://hailo-model-zoo.s3.eu-west-2.amazonaws.com/ModelZoo/Compiled/v2.19.0/hailo8/efficientdet_lite1.hef |

## Input / output (from Model Zoo network YAML + base/efficientdet_lite.yaml)

| Field | Value |
|---|---|
| Input | 384x384x3 RGB, uint8 (normalize_in_net mean=127/std=128) |
...
| Per-detection row | [ymin, xmin, ymax, xmax, score], normalized [0,1] to 384x384 input |
...
- HailoRT returns the NMS vstream as a ragged per-class list (NMS-by-score) —
  the same form as Lite0. The parser handles ragged/object/dense layouts and
  logs the raw type/shape on first inference; confirm the chosen path produces
  sensible boxes.
