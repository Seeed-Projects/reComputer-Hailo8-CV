...
# Standard COCO 80-class order. The HEF exposes 80 class slots (meta_arch=yolov8,
# 0-indexed), so cls_id maps directly to this list — no labels_offset, no gaps.
DEFAULT_CLASSES = (
...
app = FastAPI(title="reComputer NanoDet-RepVGG-a1-640 Hailo-8")
...
@app.post("/api/models/nanodet_repvgg_a1_640/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer NanoDet-RepVGG-a1-640 · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>NanoDet-RepVGG-a1-640 · RPi5 + Hailo-8</h1>
          <div class="tabs">
...
# ---------------------------------------------------------------------------
# NanoDet-RepVGG-a1-640 post-processing (on-chip NMS / HPP)
#
...
def main():
    parser = argparse.ArgumentParser(description='NanoDet-RepVGG-a1-640 on RPi5 + Hailo-8 (Web Preview Mode)')
