...
# SSD-MobileNet-V2 outputs 90 class slots (base config: classes=90,
...
app = FastAPI(title="reComputer SSD-MobileNet-V2 Hailo-8")
...
@app.post("/api/models/ssd_mobilenet_v2/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer SSD-MobileNet-V2 · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>SSD-MobileNet-V2 · RPi5 + Hailo-8</h1>
          <div class="tabs">
...
# ---------------------------------------------------------------------------
# SSD-MobileNet-V2 post-processing (on-chip NMS / HPP)
#
...
def main():
    parser = argparse.ArgumentParser(description='SSD-MobileNet-V2 on RPi5 + Hailo-8 (Web Preview Mode)')
