# DAMO-YOLO（tinynasL20_T）：Raspberry Pi 5 / CM5 + Hailo-8 目标检测

本模块运行 DAMO-YOLO 目标检测（TinyNAS-L20 主干，Tiny 尺寸，COCO 80 类），
使用 `nanodet_split` 后处理头（DFL 框回归）。FastAPI 服务支持图片、视频文件、
USB 摄像头、MJPEG 预览和 REST 推理接口。
...
|---|---|---|
| `/health` | GET | 健康检查 |
| `/api/models/damoyolo_tinynas_l20_t/predict` | POST | 检测结果（JSON） |
