...
# models, tiny_yolov4 does NOT do NMS on-device — the grid decode + sigmoid +
...
# tiny_yolov4 anchors (from network YAML). 2 strides, 3 anchor pairs each.
...
app = FastAPI(title="reComputer Tiny-YOLOv4 Hailo-8")
...
@app.post("/api/models/tiny_yolov4/predict")
...
async def index():
    ...
    <html><head><title>reComputer Tiny-YOLOv4 · Hailo-8</title>
    <style>
    ...
    <div class="container"><h1>Tiny-YOLOv4 · RPi5 + Hailo-8</h1>
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    if not _DET_OUTPUT_LOGGED:
        for i, h in enumerate(heads):
            print(f"[Tiny-YOLOv4] head{i}: shape={h.shape}", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description='Tiny-YOLOv4 on RPi5 + Hailo-8 (Web Preview Mode)')
