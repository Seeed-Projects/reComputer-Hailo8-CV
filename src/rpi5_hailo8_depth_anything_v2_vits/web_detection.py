...
IMG_SIZE = (224, 224)  # (width, height) — overridden at runtime
_DEPTH_LOGGED = False

class DetectionConfig:
    def __init__(self):
        self.obj_thresh = 0.25  # alpha blend factor for overlay
        self.nms_thresh = 0.45   # unused, API parity
...
class VideoAnalyzer:
    def __init__(self):
        self.model=None; self.co_helper=None; self.is_processing=False
        self.progress=0; self.current_video=""; self.error_msg=""
    ...
    def start_analysis(self,inp,out):
        ...
        self._stop.clear(); self._thread=threading.Thread(target=self._proc,args=(inp,out))
    ...
    def _proc(self,inp,out):
        self.is_processing=True; self.progress=0; self.error_msg=""; self.current_video=os.path.basename(inp)
            ...
            while not self._stop.is_set():
                ...
                if self.model and self.co_helper:
                    ...
                    if outputs is not None:
                        ...
                        depth=post_process_hailo(outputs,0,0,IMG_SIZE[1],IMG_SIZE[0])
                        if depth is not None:
                            real_depth=unletterbox_depth(depth,lb,h,w)
                            draw_depth(frame,real_depth,obj)
...
app=FastAPI(title="reComputer Depth-Anything-V2-ViTS Hailo-8")
...
async def update_config(c:dict):
    det_config.update(c.get("obj_thresh",0.25),c.get("nms_thresh",0.45))
...
async def analyze_video(filename:str=Form(...)):
    ...
    nb=os.path.splitext(filename)[0]; of=f"{nb}_{w}x{h}_results.mp4"; op=os.path.join(OUTPUT_DIR,of)
...
@app.post("/api/models/depth_anything_v2_vits/predict")
async def predict(file:Optional[UploadFile]=File(None),video:Optional[UploadFile]=File(None),
    ...
    try:
        ...
        if depth is not None:
            real_depth=unletterbox_depth(depth,lb,h,w)
            valid=real_depth[np.isfinite(real_depth)]
            if len(valid)>0:
                stats={"min":float(valid.min()),"max":float(valid.max()),
                       "mean":float(valid.mean()),"std":float(valid.std())}
        return {"success":True,"source":src,"depth_stats":stats,"image":{"width":w,"height":h}}
...
class FrameBuffer:
    ...
    def wait_annotated(self,v,t=1.0):
...
async def index():
    ...
    <html><head><title>reComputer Depth-Anything-V2-ViTS · Hailo-8</title>
    <style>
    ...
    <div class="container"><h1>Depth-Anything-V2-ViTS · RPi5 + Hailo-8</h1>
    <div class="tabs"><div class="tab active" onclick="showTab('rt')">Real-time Depth</div><div class="tab" onclick="showTab('an')">Local Video</div></div>
    <div id="rt" class="tab-content active">
      <div class="video-box"><img id="s" src="/api/video_feed" style="max-width:100%;height:auto;"></div>
      <div class="controls"><div class="control-group"><label>Overlay Alpha</label><div class="slider-container"><input type="range" id="c" min="0.1" max="1.0" step="0.05" value="0.6"><span id="cv" class="value-display">0.60</span></div></div></div>
    </div>
...
# ---------------------------------------------------------------------------
# Depth-Anything-V2-ViTS post-processing
#
# The HEF outputs a single-channel depth map: 224x224x1 (relative depth,
# zero-shot). No on-chip post-processing. The app:
#   1. Extracts the 224x224 depth map
#   2. Min-max normalizes to [0,1]
#   3. Applies INFERNO colormap for visualization
#   4. Alpha-blends onto the original frame
#   5. For API: returns depth stats (min, max, mean, std)
# ---------------------------------------------------------------------------

def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Extract the 224x224 depth map from the HEF output.

    Returns a 2D float32 array (224x224) of relative depth values, or None.
    """
    global _DEPTH_LOGGED
    ...
    if not _DEPTH_LOGGED:
        print(f"[Depth-Anything] raw output shape={arr.shape}, dtype={arr.dtype}", flush=True)
        _DEPTH_LOGGED = True

    # Drop batch dim, normalize to 2D (H, W)
    if arr.ndim == 4:
        arr = arr[0]  # (C, H, W) or (H, W, C)
    if arr.ndim == 3:
        if arr.shape[0] == 1:
            arr = arr[0]  # (1, H, W) → (H, W)
        elif arr.shape[-1] == 1:
            ...
            arr = arr[0] if arr.shape[0] in (arr.shape[1], arr.shape[2]) else arr[..., 0]
    if arr.ndim != 2:
        print(f"[Depth-Anything] unexpected shape: {arr.shape}", flush=True)
        return None
...
def unletterbox_depth(depth, lb_info, orig_h, orig_w):
    """Resize the depth map from model input space (224x224, letterboxed)
    to the original frame size, cropping the letterbox padding."""
    if depth is None:
        return None
    ratio, dw, dh = lb_info
    H, W = depth.shape

    # Crop the content region (undo letterbox padding)
    top = int(round(dh - 0.1))
    bottom = H - int(round(dh + 0.1)) if dh > 0 else H
    left = int(round(dw - 0.1))
    right = W - int(round(dw + 0.1)) if dw > 0 else W
    content = depth[top:bottom, left:right]

    # Resize to original frame
    real_depth = cv2.resize(content, (orig_w, orig_h), interpolation=cv2.INTER_NEAREST)
    return real_depth

def draw_depth(image, depth, alpha):
    """Apply INFERNO colormap to the depth map and alpha-blend onto the image."""
    if depth is None:
        return
    ...
    if dmax - dmin < 1e-6:
        ...
        norm = ((depth - dmin) / (dmax - dmin) * 255).clip(0, 255).astype(np.uint8)

    # Apply colormap (INFERNO: dark=close, bright=far)
    colored = cv2.applyColorMap(norm, cv2.COLORMAP_INFERNO)

    # Alpha blend
    a = np.clip(alpha, 0.0, 1.0)
    blended = cv2.addWeighted(image, 1.0 - a, colored, a, 0)
    image[:] = blended

def preprocess_frame(frame, co_helper):
    """Letterbox + BGR to RGB. Input 224x224.
    ...
    try:
        while not stop_event.is_set():
            ...
            if outputs is not None:
                ...
                if depth is not None:
                    h, w = frame.shape[:2]
                    real_depth = unletterbox_depth(depth, lb_info, h, w)
                    draw_depth(frame, real_depth, obj)
...
def main():
    parser = argparse.ArgumentParser(description='Depth-Anything-V2-ViTS on RPi5 + Hailo-8')
