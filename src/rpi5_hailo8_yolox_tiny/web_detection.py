...
# YOLOX-Tiny outputs 80 classes (base config: classes=80,
...
app = FastAPI(title="reComputer YOLOX-Tiny Hailo-8")
...
@app.post("/api/models/yolox_tiny/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer YOLOX-Tiny · Hailo-8</title>
        <style>
...
# ---------------------------------------------------------------------------
# YOLOX-Tiny post-processing (on-chip NMS / HPP)
#
...
def main():
    parser = argparse.ArgumentParser(description='YOLOX-Tiny on RPi5 + Hailo-8 (Web Preview Mode)')
