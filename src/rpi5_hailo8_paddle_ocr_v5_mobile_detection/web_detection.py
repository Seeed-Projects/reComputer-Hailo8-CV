...
# DB text detector thresholds (PaddleOCR defaults). The Model Zoo network
# YAML does not publish eval thresholds for the DB head; 0.30/0.60 are the
# standard PaddleOCR det_db values and are exposed as live sliders.
OBJ_THRESH = 0.60    # box score threshold (mean heatmap value inside box)
NMS_THRESH = 0.30    # binarization threshold on the probability map
IMG_SIZE = (960, 544)  # (width, height) — overridden at runtime from the .hef
    ...
    def _process_video(self, input_path, output_path):
        ...
        try:
            while not self._stop_event.is_set():
                ...
                if self.model:
                    try:
                        polygons = predict_polygons(frame)
                        draw(frame, polygons)
...
app = FastAPI(title="reComputer PaddleOCR v5 Mobile Detection Hailo-8")
...
# ---------------------------------------------------------------------------
# PaddleOCR DB text detection (CPU post-processing)
#
# The HEF outputs a single probability map (H/4 x W/4, values in [0,1]).
# Post-processing follows the DB (Differentiable Binarization) head from
# PaddleOCR: binarize the heatmap, extract contours, min-area-rect, score
# filter, unclip expansion, then scale boxes back to the original frame.
# Ported from the module's py_utils/db_postprocess.py (Apache-2.0).
# ---------------------------------------------------------------------------

_model_lock = threading.Lock()


def preprocess_frame(frame, co_helper=None):
    """Resize the raw frame to the model input (DB uses a plain resize, no
    letterbox). Returns (img, (sx, sy)) with the exact scale factors."""
    ih, iw = IMG_SIZE[1], IMG_SIZE[0]
    sx = float(iw) / frame.shape[1]
    sy = float(ih) / frame.shape[0]
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """DB decode. Returns (polygons, scores, classes):
      polygons (N, 4, 2) int32 quadrilaterals in original-frame pixels
      scores   (N,) mean heatmap value inside each box
      classes  (N,) all 0 (single "text" class)
    nms_thresh is the binarization threshold; obj_thresh is the box score.
    """
    ...
    arr = arr.squeeze()
    if arr.ndim != 2:
        print(f"[PaddleOCR-det] unexpected output shape: {arr.shape}", flush=True)
        return None, None, None
    # Work in input-pixel space: scale the heatmap up to input size so
    # detect_boxes maps boxes back with a single known scale.
    heatmap = cv2.resize(arr, (input_w, input_h))
    # Reuse the DB postprocess on a fake "frame" of the input size, then the
    # returned boxes are already in input pixels; caller scales to original.
    fake = np.zeros((input_h, input_w, 3), dtype=np.uint8)
    boxes = detect_boxes(heatmap, fake, nms_thresh, obj_thresh)
    if not boxes:
        return None, None, None
    polygons = np.asarray(boxes, dtype=np.float32)  # (N, 4, 2) input px
    scores = np.ones(len(polygons), dtype=np.float32)
    classes = np.zeros(len(polygons), dtype=np.int32)
    return polygons, scores, classes


def scale_polygons(polygons, lb_info, frame_shape):
    """Map (N, 4, 2) polygons from input-pixel space to the original frame.
    lb_info = (sx, sy) are the forward scale factors."""
...
def draw(image, polygons, scores=None, classes=None):
    """Draw detected text regions as green quadrilaterals."""
    if polygons is None or len(polygons) == 0:
        return
    for poly in polygons:
        pts = np.asarray(poly, dtype=np.int32).reshape(-1, 2)
        cv2.polylines(image, [pts], True, (58, 197, 92), 2)


def predict_polygons(frame):
    """Shared inference path for the live loop and the analyzer."""
    model = _global_model
    if model is None:
        return None
    processed_img, lb_info = preprocess_frame(frame)
    with _model_lock:
        outputs = model.run(processed_img)
    obj, nms = det_config.get()
    polygons, _scores, _classes = post_process_hailo(
        outputs, obj, nms, IMG_SIZE[1], IMG_SIZE[0])
    return scale_polygons(polygons, lb_info, frame.shape)


@app.post("/api/models/paddle_ocr_v5_mobile_detection/predict")
async def predict(
    ...
    try:
        if conf is not None or iou is not None:
            cur_obj, cur_nms = det_config.get()
            det_config.update(
                conf if conf is not None else cur_obj,
                iou if iou is not None else cur_nms)
        img = None
        source_info = ""
        if file:
        ...
        if polygons is not None and len(polygons) > 0:
            for poly in polygons:
                ...
                predictions.append({
                    "class": "text",
                    "polygon": [[int(px), int(py)] for px, py in poly],
                    "box": {"x1": x1, "y1": y1, "x2": x2, "y2": y2,
                            "width": x2 - x1, "height": y2 - y1},
                })
        return {"success": True, "source": source_info, "predictions": predictions,
                "count": len(predictions), "image": {"width": w, "height": h}}
    except Exception as e:
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer PaddleOCR Detection · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>PaddleOCR v5 Mobile Detection · RPi5 + Hailo-8</h1>
          <div class="tabs">
          ...
          <div id="realtime" class="tab-content active">
            ...
            <div class="controls">
              <div class="control-group">
                <label>Box Score Threshold</label>
                <div class="slider-container">
                  <input type="range" id="confSlider" min="0.01" max="1.0" step="0.01" value="0.60">
                  <span id="confValue" class="value-display">0.60</span>
                </div>
              </div>
              <div class="control-group">
                <label>Binarization Threshold</label>
                <div class="slider-container">
                  <input type="range" id="iouSlider" min="0.01" max="1.0" step="0.01" value="0.30">
                  <span id="iouValue" class="value-display">0.30</span>
                </div>
              </div>
            </div>
          </div>
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                ...
                print(f"[PaddleOCR-det] inference error: {e}", flush=True)
            ...
            n_regions = 0 if polygons is None else len(polygons)
            cv2.putText(frame, f'Hailo FPS: {fps_counter:.1f} | text regions: {n_regions}', (20, 40),
...
def main():
    parser = argparse.ArgumentParser(description='PaddleOCR v5 Mobile Detection on RPi5 + Hailo-8 (Web Preview Mode)')
    ...
    inf_thread = threading.Thread(target=inference_loop,
                                  args=(capture_source, model, None, is_video_file, args.target_fps),
                                  daemon=True)
