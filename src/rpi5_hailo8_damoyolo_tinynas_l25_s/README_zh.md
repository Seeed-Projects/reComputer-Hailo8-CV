# DAMO-YOLO（tinynasL25_S）：Raspberry Pi 5 / CM5 + Hailo-8 目标检测

本模块运行 DAMO-YOLO 目标检测（TinyNAS-L25 主干，Small 尺寸，COCO 80 类），
使用 `nanodet_split` 后处理头（DFL 框回归）。它是 damoyolo 家族中精度更高的
Small（L25_S）变体，与 L20_T 输入输出契约相同，主干更深。FastAPI 服务支持
...
|---|---|---|
| `/health` | GET | 健康检查 |
| `/api/models/damoyolo_tinynas_l25_s/predict` | POST | 检测结果（JSON） |
| `/api/video_feed` | GET | MJPEG 预览流 |

## 实现说明

- 输入输出契约和后处理与 L20_T 完全一致，仅主干不同（TinyNAS-L25，Small
  尺寸，更准但更重）。
