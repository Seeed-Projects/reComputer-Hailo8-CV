...
# Demo defaults. The Model Zoo eval uses score_threshold=0.05 and
# nms_iou_thresh=0.7 (base/damoyolo.yaml); 0.25/0.45 give a cleaner live preview
# — lower the sliders to inspect lower-confidence detections.
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (640, 640)  # (width, height) — overridden at runtime from the .hef

# Standard COCO 80-class order (Model Zoo COCO_2017_TO_2014_TRANSLATION confirms
# the channel ordering). labels_offset is eval-only and does not affect naming.
DEFAULT_CLASSES = (
...
app = FastAPI(title="reComputer DAMO-YOLO (tinynasL25_S) Hailo-8")
...
@app.post("/api/models/damoyolo_tinynas_l25_s/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer DAMO-YOLO (tinynasL25_S) · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>DAMO-YOLO (tinynasL25_S) · RPi5 + Hailo-8</h1>
...
# ---------------------------------------------------------------------------
# DAMO-YOLO (nanodet_split) post-processing
#
# The HEF (damoyolo_tinynasL25_S) exposes six FLOAT32 outputs, three scales
# x (box, cls), matching the Model Zoo network YAML output_shape:
#   80x80x68, 80x80x81, 40x40x68, 40x40x81, 20x20x68, 20x20x81
# Per scale (strides 8/16/32):
#   box = 68ch = 4 x (reg_max+1) = 4x17 DFL distributions for l,t,r,b
#   cls = 81ch = 80 classes + 1 (take first 80); sigmoid applied on-chip
#
# Decode follows the official Hailo Model Zoo
# core/postprocessing/detection/nanodet.py (split_decode + _box_decoding):
#   softmax over the 17 bins per coord, weighted sum by [0..reg_max] -> distance
#   in stride units, * stride -> pixels; box = [cx-l, cy-t, cx+r, cy+b]
#   with center = grid_index * stride. Only sigmoid is on-chip; the grid decode,
#   DFL reduction and NMS run on the CPU.
# ---------------------------------------------------------------------------

DAMOYOLO_STRIDES = (8, 16, 32)
DAMOYOLO_REG_MAX = 16  # base/damoyolo.yaml regression_length -> 4*(16+1)=68
...
def _nms_per_class(boxes, scores, classes, iou_thresh, max_det=100):
    """Per-class NMS (matches Model Zoo combined_non_max_suppression), then
    global top-k by score. Boxes of different classes may overlap and survive."""
    keep = []
    for c in np.unique(classes):
        idx = np.where(classes == c)[0]
        k = _nms(boxes[idx], scores[idx], iou_thresh, max_det)
        keep.extend(idx[k].tolist())
    keep = np.array(keep, dtype=np.int64)
    if len(keep) > max_det:
        order = scores[keep].argsort()[::-1][:max_det]
        keep = keep[order]
    return keep
...
def _normalize_feature_tensor(tensor):
    """Drop the batch dim and move channels last if the layout looks like CHW.
    Each scale exposes two heads: 68-channel box (4x17 DFL) and 80/81-channel
    cls, both disambiguated by channel count in _collect_damoyolo_heads."""
    arr = np.asarray(tensor)
    if arr.ndim == 4 and arr.shape[0] == 1:
        arr = arr[0]
    if arr.ndim == 3 and arr.shape[0] in (68, 80, 81) and arr.shape[-1] not in (68, 80, 81):
        arr = np.moveaxis(arr, 0, -1)
    return arr.astype(np.float32, copy=False)


def _collect_damoyolo_heads(hailo_output):
    """Group the six outputs into (box, cls) pairs per scale, ordered by
    spatial size descending (= strides 8, 16, 32). At each scale the tensor
    with fewer channels is the box head (68 = 4x17); the one with more is cls
    (80/81). This is robust to HailoRT dict ordering; first-inference logging
    (SOP §10) records every vstream name/shape for on-device verification."""
    ...
    for name, tensor in _iter_output_tensors(hailo_output):
        ...
        if arr.ndim != 3:
            continue
        by_spatial[arr.shape[:2]].append(arr)

    heads = []
    for spatial in sorted(by_spatial.keys(), key=lambda s: s[0] * s[1], reverse=True):
        group = by_spatial[spatial]
        if len(group) < 2:
            continue
        group.sort(key=lambda a: a.shape[-1])  # box (fewer ch) first, cls second
        heads.append((group[0], group[1]))

    if len(heads) < len(DAMOYOLO_STRIDES):
        return [], seen
    return heads[:len(DAMOYOLO_STRIDES)], seen
...
def _decode_damoyolo(heads, obj_thresh, nms_thresh, input_h, input_w, max_det=100):
    ...
    reg_max = DAMOYOLO_REG_MAX
    reg_range = np.arange(reg_max + 1, dtype=np.float32)
    num_classes = len(CLASSES)

    all_boxes, all_scores, all_classes = [], [], []
    for (box, cls), stride in zip(heads, DAMOYOLO_STRIDES):
        H, W, _ = box.shape
        N = H * W

        # DFL: softmax over the 17 bins per coordinate, weighted sum -> expected
        # distance in stride units, then * stride -> pixels.
        box_dist = box.reshape(N, 4, reg_max + 1)
        box_dist = _softmax(box_dist, axis=-1)
        box_dist = (box_dist * reg_range).sum(axis=-1)  # (N, 4) [l, t, r, b]
        box_dist = box_dist * stride
        box_dist[:, :2] *= -1  # -> [-l, -t, +r, +b] offsets from center

        # Grid centers: cx = col*stride, cy = row*stride (row-major to match
        # the C-order reshape above).
        gy, gx = np.meshgrid(np.arange(H, dtype=np.float32),
                             np.arange(W, dtype=np.float32), indexing='ij')
        cx = gx.flatten() * stride
        cy = gy.flatten() * stride
        ...
        if not mask.any():
            continue
        cells, class_idx = np.where(mask)
        all_boxes.append(boxes[cells])
        all_scores.append(cls_arr[cells, class_idx])
        all_classes.append(class_idx.astype(np.int32))
    ...
    keep = _nms_per_class(boxes, scores, classes, nms_thresh, max_det=max_det)
    if not len(keep):
        return None, None, None
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Decode the six DAMO-YOLO heads into boxes/classes/scores.

    Returns (boxes, classes, scores) where boxes are xyxy in input-pixel space
    (the 640x640 letterboxed input). The caller un-letterboxes to the original
    ...
    heads, seen = _collect_damoyolo_heads(hailo_output)
    if not _DET_OUTPUT_LOGGED:
        print(f"[DAMO-YOLO] outputs: {'; '.join(seen)}", flush=True)
        for i, (box, cls) in enumerate(heads):
            print(
                f"[DAMO-YOLO] scale{i} stride={DAMOYOLO_STRIDES[i]}: "
                f"box={tuple(box.shape)} cls={tuple(cls.shape)}",
                flush=True,
            )
        _DET_OUTPUT_LOGGED = True
    return _decode_damoyolo(heads, obj_thresh, nms_thresh, input_h, input_w)


def unletterbox_boxes(boxes, lb_info):
    """Map xyxy boxes from the 640x640 letterboxed input back to the original
...
def preprocess_frame(frame, co_helper):
    ...
    The HEF uses normalize_in_net with mean=0/std=1 (a no-op) and padding_color
    0, so we feed raw uint8 RGB pixels after letterboxing — no manual
    normalization.
    """
...
def main():
    parser = argparse.ArgumentParser(description='DAMO-YOLO (tinynasL25_S) on RPi5 + Hailo-8 (Web Preview Mode)')
