# CenterNet（resnet_v1_50）：Raspberry Pi 5 / CM5 + Hailo-8 目标检测

本模块运行 CenterNet 目标检测（ResNet-50 主干，COCO 80 类），使用片上
`max_finder` 后处理变体。它是 resnet_v1_18 的高精度版本：输入输出契约相同，
主干更深。FastAPI 服务支持图片、视频文件、USB 摄像头、MJPEG 预览和 REST
推理接口。
...
|---|---|---|
| `/health` | GET | 健康检查 |
| `/api/models/centernet_resnet_v1_50_postprocess/predict` | POST | 检测结果（JSON） |
| `/api/video_feed` | GET | MJPEG 预览流 |

## 实现说明

- 输入输出契约和后处理与 resnet_v1_18 完全一致，仅主干不同（ResNet-50，
  更准但更重）。
