...
# Default confidence/IoU. obj_thresh 0.2 matches the model's on-chip
# score_threshold (centernet.yaml base config); the HEF heatmap is already
# sparse above it, so 0.2 surfaces everything the device emits.
OBJ_THRESH = 0.20
NMS_THRESH = 0.45
IMG_SIZE = (512, 512)  # (width, height) — overridden at runtime from the .hef

# Standard COCO 80-class order. The heatmap channel index maps directly to this
# list (verified against Model Zoo COCO_2017_TO_2014_TRANSLATION). The +1
# label_offset in the official postproc is for mAP evaluation only.
DEFAULT_CLASSES = (
...
app = FastAPI(title="reComputer CenterNet (resnet_v1_50) Hailo-8")
...
@app.post("/api/models/centernet_resnet_v1_50_postprocess/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer CenterNet (resnet_v1_50) · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>CenterNet (resnet_v1_50) · RPi5 + Hailo-8</h1>
            ...
            <div class="controls">
              <div class="control-group">
                ...
                <div class="slider-container">
                  <input type="range" id="confSlider" min="0.01" max="1.0" step="0.01" value="0.20">
                  <span id="confValue" class="value-display">0.20</span>
                </div>
              </div>

              <div class="control-group">
                <label>IOU Threshold (NMS cleanup)</label>
                <div class="slider-container">
...
# ---------------------------------------------------------------------------
# CenterNet post-processing
#
# The HEF (centernet_resnet_v1_50_postprocess) exposes three FLOAT32 outputs,
# matching the Model Zoo network YAML output_nodes order:
#   [0] centernet0_conv3  -> wh   (128x128x2)  box width/height, stride units
#   [1] centernet0_conv5  -> reg  (128x128x2)  sub-pixel center offset
#   [2] threshold_confidence/.../Relu -> hm   (128x128x80) sparse heatmap,
#        already processed by the on-chip max_finder + score_threshold(0.2).
#
# Decode follows the official Hailo Model Zoo
# core/postprocessing/detection/centernet.py::_generate_boxes exactly:
#   SCALE = input_size / feat_size  (= 512 / 128 = 4)
#   center = SCALE * (feat_coord + reg)
#   box_w  = SCALE * wh_x ;  box_h = SCALE * wh_y
#   box    = [center - 0.5*box, center + 0.5*box]   (in model-pixel space)
# The official code normalizes by (SCALE * feat_size) = input_size; we keep
# model pixels and un-letterbox afterwards, which is equivalent.
# ---------------------------------------------------------------------------
...
def _normalize_feature_tensor(tensor):
    """Drop the batch dim and, if the layout looks like CHW, move channels last.
    HailoRT may hand back NCHW or NHWC; both 2-channel (wh/reg) and 80-channel
    (hm) heads are disambiguated by channel count."""
    arr = np.asarray(tensor)
    if arr.ndim == 4 and arr.shape[0] == 1:
        arr = arr[0]
    if arr.ndim == 3 and arr.shape[0] in (2, 80) and arr.shape[-1] not in (2, 80):
        arr = np.moveaxis(arr, 0, -1)
    return arr.astype(np.float32, copy=False)


def _collect_centernet_heads(hailo_output):
    """Map the three CenterNet vstreams to hm / wh / reg.

    The heatmap is identified unambiguously by its 80 channels. The two
    2-channel heads (wh, reg) are assigned by vstream name when the conv
    number is recoverable (conv3 -> wh, conv5 -> reg), otherwise by network
    output order — which matches the official `outputs = endnodes` path for
    the max_finder variant. First-inference logging (SOP §10) records every
    vstream name/shape so the mapping can be verified on hardware.
    """
    heads = {}
    two_channel = []
    seen = []
    ...
    for name, tensor in _iter_output_tensors(hailo_output):
        ...
        if channels == 2:
            two_channel.append((name, arr))
        elif channels > max_channels:
            max_channels = channels
            heads["hm"] = arr

    wh_by_name = reg_by_name = None
    for name, arr in two_channel:
        lname = str(name).lower()
        if "conv3" in lname and wh_by_name is None:
            wh_by_name = arr
        elif "conv5" in lname and reg_by_name is None:
            reg_by_name = arr
    if wh_by_name is not None and reg_by_name is not None:
        heads["wh"] = wh_by_name
        heads["reg"] = reg_by_name
    elif len(two_channel) >= 2:
        # Fallback: network output order is [wh, reg, hm].
        heads["wh"] = two_channel[0][1]
        heads["reg"] = two_channel[1][1]
    elif len(two_channel) == 1:
        heads["wh"] = two_channel[0][1]

    return heads, seen


def _local_max_suppress(heatmap):
    """Per-channel 3x3 local-max suppression.

    The HEF already applies on-chip max_finder, so this is idempotent on the
    sparse tensor (peaks survive; zeros stay zero). It is kept as a defensive
    measure in case a particular HEF build returns a dense heatmap. Vectorized
    via cv2.dilate (handles arbitrary channel counts), with a per-channel
    fallback if OpenCV refuses a wide multi-channel array.
    """
    if heatmap.ndim != 3:
        return heatmap
    ...
    try:
        ...
        return heatmap * (heatmap >= dilated - 1e-7)
    except cv2.error:
        ...
        for c in range(heatmap.shape[-1]):
            ...
            if ch.max() <= 0:
                continue
            d = cv2.dilate(ch, kernel)
            out[..., c] = ch * (ch >= d - 1e-7)
        return out


def _decode_centernet(heads, obj_thresh, nms_thresh, input_h, input_w, max_det=100):
    required = ("hm", "wh", "reg")
    ...
    # The on-chip threshold/max_finder emits confidences in [0,1]; apply sigmoid
    # only if the tensor looks like raw logits (defensive — should be a no-op).
    if heatmap.min() < 0.0 or heatmap.max() > 1.0:
        heatmap = _sigmoid(heatmap)
    heatmap = _local_max_suppress(heatmap)

    feat_h, feat_w, _ = heatmap.shape
    scale = input_w / float(feat_w)  # 512 / 128 = 4.0

    # Peaks: every non-zero channel entry above the confidence threshold. The
    # official code takes global top-K (100); we replicate that cap.
    mask = heatmap >= obj_thresh
    ys, xs, cs = np.where(mask)
    if ys.size == 0:
        return None, None, None
    scores = heatmap[ys, xs, cs]
    if scores.size > max_det:
        order = np.argsort(scores)[::-1][:max_det]
        ys, xs, cs, scores = ys[order], xs[order], cs[order], scores[order]

    wh_vals = wh[ys, xs]      # (N, 2)
    reg_vals = reg[ys, xs]    # (N, 2)

    # Centers and box dims in model-pixel space (letterboxed 512x512 input).
    cx = scale * (xs + reg_vals[:, 0])
    cy = scale * (ys + reg_vals[:, 1])
    bw = scale * wh_vals[:, 0]
    bh = scale * wh_vals[:, 1]
    boxes = np.stack([
        cx - 0.5 * bw,
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Decode the three CenterNet HEF output heads into boxes/classes/scores.

    Returns (boxes, classes, scores) where boxes are xyxy in model-pixel space
    (the 512x512 letterboxed input). The caller un-letterboxes to the original
    ...
    heads, seen = _collect_centernet_heads(hailo_output)
    if not _DET_OUTPUT_LOGGED:
        hm = heads.get("hm")
        wh = heads.get("wh")
        reg = heads.get("reg")
        print(f"[CenterNet] outputs: {'; '.join(seen)}", flush=True)
        print(
            f"[CenterNet] heads: hm={getattr(hm, 'shape', None)}, "
            f"wh={getattr(wh, 'shape', None)}, reg={getattr(reg, 'shape', None)}",
            flush=True,
        )
        _DET_OUTPUT_LOGGED = True
    return _decode_centernet(heads, obj_thresh, nms_thresh, input_h, input_w)


def unletterbox_boxes(boxes, lb_info):
    """Map xyxy boxes from the 512x512 letterboxed input back to the original
...
def draw(image, boxes, scores, classes):
    for box, score, cl in zip(boxes, scores, classes):
        ...
        cv2.putText(image, f'{CLASSES[int(cl)]} {float(score):.2f}',
...
def preprocess_frame(frame, co_helper):
    ...
    exact ratio + padding used for this frame so boxes can be un-letterboxed
    ...
    The HEF was compiled with normalize_in_net=true (ImageNet mean/std, RGB),
    so we feed raw uint8 RGB pixels — no manual normalization here.
    """
...
def main():
    parser = argparse.ArgumentParser(description='CenterNet (resnet_v1_50_postprocess) on RPi5 + Hailo-8 (Web Preview Mode)')
    ...
    # Pull the real (H, W) out of the .hef so any input resolution works
    # without editing constants. Stored as (W, H) to match the rest of the file.
