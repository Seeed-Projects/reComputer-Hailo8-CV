...
# nms_iou_thresh=0.5 (base/efficientdet_lite.yaml); 0.25 gives a cleaner live
# preview — lower the slider to inspect lower-confidence boxes.
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (320, 320)  # (width, height) — overridden at runtime from the .hef

# EfficientDet-Lite outputs 89 class slots (base config: classes=89,
# labels_offset=1). The official tf_postproc_nms maps cls_id -> COCO category
# id (cls_id + 1). This list is indexed by cls_id (0..88) with COCO category
# IDs 1..89; the 10 unused COCO IDs (12, 26, 29, 30, 45, 66, 68, 69, 71, 83)
...
app = FastAPI(title="reComputer EfficientDet-Lite2 Hailo-8")
...
@app.post("/api/models/efficientdet_lite2/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer EfficientDet-Lite2 · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>EfficientDet-Lite2 · RPi5 + Hailo-8</h1>
          <div class="tabs">
...
# ---------------------------------------------------------------------------
# EfficientDet-Lite2 post-processing (on-chip NMS / HPP)
#
...
# YAML the post-NMS shape is 89x5x100 = (num_classes, 5, max_dets); the
...
#   2. dense float32 (1, 89, 5, 100): transpose to (1, 89, 100, 5).
#   3. dense float32 (1, 89, 100, 5): already per-class-per-det.
...
def preprocess_frame(frame, co_helper):
    ...
    The HEF uses normalize_in_net with mean=127/std=128 ((pixel-127)/128) and
...
def main():
    parser = argparse.ArgumentParser(description='EfficientDet-Lite2 on RPi5 + Hailo-8 (Web Preview Mode)')
