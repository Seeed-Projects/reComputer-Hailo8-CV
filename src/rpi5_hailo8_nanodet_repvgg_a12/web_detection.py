...
# (kept for API parity). The Model Zoo eval uses score_threshold=0.05 /
# nms_iou_thresh=0.6 (base/nanodet.yaml); 0.25 gives a cleaner live
...
# Standard COCO 80-class order. The HEF exposes 80 class slots (meta_arch=yolox,
# 0-indexed; meta_arch=yolox with labels_offset=1 (eval-only) — cls_id maps directly to this list. Verify on hardware (SOP §10).
DEFAULT_CLASSES = (
...
app = FastAPI(title="reComputer NanoDet-RepVGG-a12 Hailo-8")
...
@app.post("/api/models/nanodet_repvgg_a12/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer NanoDet-RepVGG-a12 · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>NanoDet-RepVGG-a12 · RPi5 + Hailo-8</h1>
          <div class="tabs">
...
# ---------------------------------------------------------------------------
# NanoDet-RepVGG-a12 post-processing (on-chip NMS / HPP)
#
...
# YAML the post-NMS shape is 80x5x100 = (num_classes, 5, max_dets); the
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    if not _DET_OUTPUT_LOGGED:
        ...
        print(
            f"[NanoDet] raw output type={type(output).__name__}, "
...
def preprocess_frame(frame, co_helper):
    """Letterbox, keep BGR. Returns (img, lb_info) where lb_info captures the
    exact ratio + padding for un-letterboxing independent of shared co_helper
    state.

    The HEF uses normalize_in_net (ImageNet RGB mean/std, compiled in) plus an
    on-chip bgr_to_rgb conversion (input_conversion in the .alls), so the app
    feeds raw uint8 BGR pixels after letterboxing — no manual normalization,
    no cvtColor.
    """
...
def main():
    parser = argparse.ArgumentParser(description='NanoDet-RepVGG-a12 on RPi5 + Hailo-8 (Web Preview Mode)')
