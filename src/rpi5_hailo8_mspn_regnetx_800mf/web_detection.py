...
NMS_THRESH = 0.45  # unused (single-person pose, no NMS), kept for API parity
IMG_SIZE = (192, 256)  # (width, height) — overridden at runtime from the HEF
...
app = FastAPI(title="reComputer MSPN RegNetX-800MF Hailo-8")
...
async def index():
    ...
    <html><head><title>reComputer MSPN RegNetX-800MF · Hailo-8</title>
    <style>
    ...
    <div class="container"><h1>MSPN RegNetX-800MF · RPi5 + Hailo-8</h1>
...
# ---------------------------------------------------------------------------
# MSPN RegNetX-800MF post-processing (heatmap argmax → keypoints → skeleton)
#
# Hailo Model Zoo mspn_regnetx_800mf: input 256x192x3, output 64x48x17
# (H, W, K=17 COCO keypoints), ImageNet normalization compiled into the HEF.
# Single-person model, no boxes and no on-chip NMS. The decode:
#   1. For each of the 17 channels: argmax → (y, x) in 64x48 heatmap space
#   2. Score = max value of the channel
#   3. Scale to letterboxed input space (256x192)
#   4. Un-letterbox to the original frame
#   5. Draw keypoints + skeleton
# The raw output layout is logged once at first inference and verified on
# hardware; only the exact vstream shape may differ per Model Zoo version.
# ---------------------------------------------------------------------------

def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Decode the MSPN 64x48x17 heatmap into 17 COCO keypoints.

    Returns (17, 3) = [x, y, score] in input-pixel space, or None on failure.
    """
    global _DET_LOGGED
    ...
    # Pick the keypoint heatmap tensor. The Model Zoo network has a single
    # output, but if a HEF exposes more, prefer the one with a 17 axis.
    if isinstance(hailo_output, dict):
        output = None
        for v in hailo_output.values():
            a = np.asarray(v)
            if a.ndim >= 3 and 17 in (a.shape[-1], a.shape[1]):
                output = v; break
        if output is None:
    ...
    try:
        ...
        if not _DET_LOGGED:
            print(f"[MSPN] np.asarray failed: {e}", flush=True)
            _DET_LOGGED = True
        return None
    if not _DET_LOGGED:
        print(f"[MSPN] raw output shape={arr.shape}, dtype={arr.dtype}", flush=True)
        _DET_LOGGED = True
    ...
    if arr.ndim == 3 and arr.shape[-1] == 17:
        arr = np.moveaxis(arr, -1, 0)  # (H, W, K) → (K, H, W)
    if arr.ndim != 3 or arr.shape[0] != 17:
        print(f"[MSPN] unexpected shape after normalize: {arr.shape}", flush=True)
        return None
    ...
    for k in range(K):
        ...
        y = int(idx // W)
        x = int(idx % W)
        # Scale to letterboxed input space
        keypoints[k] = [x * input_w / W, y * input_h / H, score]

    return keypoints

def unletterbox_keypoints(kpts, lb_info):
    """Map keypoints from letterboxed input space to the original frame."""
    if kpts is None:
        return kpts
    ratio, dw, dh = lb_info
    out = kpts.copy()
...
def preprocess_frame(frame, co_helper):
    ...
    The HEF has normalize_in_net with ImageNet RGB mean/std, so feed raw
    uint8 RGB after letterboxing — no manual normalization, no cvtColor
    beyond BGR→RGB.
    """
        ...
        while not stop_event.is_set():
            ...
            if outputs is not None:
                try:
                    ...
                    print(f"[MSPN] post-process error: {e}", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description='MSPN RegNetX-800MF on RPi5 + Hailo-8')
    ...
    if args.class_path: pass  # MSPN has fixed COCO keypoints
