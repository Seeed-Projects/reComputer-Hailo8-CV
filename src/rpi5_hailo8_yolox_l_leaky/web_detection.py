...
# (kept for API parity). The Model Zoo eval uses score_threshold=0.01 /
# nms_iou_thresh=0.65 (base/yolox.yaml); 0.25 gives a cleaner live
...
# YOLOX-L-Leaky outputs 80 classes (base config: classes=80,
...
app = FastAPI(title="reComputer YOLOX-L-Leaky Hailo-8")
...
@app.post("/api/models/yolox_l_leaky/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer YOLOX-L-Leaky · Hailo-8</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>YOLOX-L-Leaky · RPi5 + Hailo-8</h1>
          <div class="tabs">
...
# ---------------------------------------------------------------------------
# YOLOX-L-Leaky post-processing (on-chip NMS / HPP)
#
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    if not _DET_OUTPUT_LOGGED:
        ...
        print(
            f"[YOLOX] raw output type={type(output).__name__}, "
...
def main():
    parser = argparse.ArgumentParser(description='YOLOX-L-Leaky on RPi5 + Hailo-8 (Web Preview Mode)')
