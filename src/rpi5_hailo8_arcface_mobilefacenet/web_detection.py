...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (112, 112)  # (width, height); refreshed from the HEF input at startup
EMBEDDING_DIM = 512
_ARC_OUTPUT_LOGGED = False

def load_classes(path):
    """ArcFace returns a 512-dim embedding and has no class label table."""
    if path:
        print(
            "Note: ArcFace MobileFaceNet produces face embeddings; "
            "--class_path is ignored.",
            flush=True,
        )
...
app = FastAPI(title="reComputer ArcFace MobileFaceNet (RPi5 + Hailo-8)")
...
@app.post("/api/models/arcface_mobilefacenet/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer ArcFace MobileFaceNet</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>ArcFace MobileFaceNet · RPi5 + Hailo-8</h1>
...
def _embedding_tensor(hailo_output):
    """Pick the embedding vstream: prefer the fc1 head, else the first output."""
    best = None
    for name, tensor in _iter_output_tensors(hailo_output):
        arr = np.asarray(tensor)
        if best is None:
            best = arr
        if str(name).rstrip("/").endswith("fc1"):
            return arr
    return best


def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """Extract the 512-dim ArcFace face embedding from the Hailo vstreams."""
    global _ARC_OUTPUT_LOGGED

    if hailo_output is None:
        return None

    if not _ARC_OUTPUT_LOGGED:
        seen = "; ".join(
            f"{name}:{tuple(np.asarray(t).shape)}" for name, t in _iter_output_tensors(hailo_output)
        )
        print(f"[ArcFace] outputs: {seen}", flush=True)
        _ARC_OUTPUT_LOGGED = True

    tensor = _embedding_tensor(hailo_output)
    if tensor is None:
        return None
    embedding = np.asarray(tensor, dtype=np.float32).reshape(-1)
...
def draw_embedding_status(image, embedding):
    """Annotate the preview without pretending an embedding is a detection."""
    if embedding is None:
        return image
    cv2.putText(
        image,
...
def preprocess_frame(frame, co_helper):
    """Letterbox to 112x112 and convert BGR to RGB.

    The HEF carries its own normalization (an in-net normalization layer), so
    the app feeds raw uint8 pixels in RGB order.
    """
...
def main():
    parser = argparse.ArgumentParser(description='ArcFace MobileFaceNet on RPi5 + Hailo-8 (Web Preview Mode)')
    ...
    # Take the real (W, H) from the HEF instead of trusting the constant.
